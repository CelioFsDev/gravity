...
class ProductsScreen extends ConsumerWidget {
  ...
  Widget _buildContent(BuildContext context, WidgetRef ref, ProductsState state) {
    return SingleChildScrollView(
       ...
       child: Column(
         ...
         children: [
           ...
           Row(
             ...
             children: [
               Column(
                 crossAxisAlignment: CrossAxisAlignment.start,
                 children: [
                   Text('Produtos', style: Theme.of(context).textTheme.headlineMedium?.copyWith(fontWeight: FontWeight.bold)),
                   Text('Catálogo de produtos', style: Theme.of(context).textTheme.bodyMedium?.copyWith(color: Colors.grey)),
                 ],
               ),
               Row(
                 children: [
                   ...
                   OutlinedButton.icon(
                     onPressed: () {
                         Navigator.of(context).push(MaterialPageRoute(builder: (_) => const ProductImportScreen()));
                     }, 
                     icon: const Icon(Icons.upload_file),
                     label: const Text('Importar')
                   ),
                   const SizedBox(width: 8),
                   OutlinedButton.icon(onPressed: (){}, icon: const Icon(Icons.download), label: const Text('Exportar')),
                 ],
               ),
             ],
           ),
           ...
           Row(
             children: [
               Expanded(child: _buildKpiCard(context, 'Total Produtos', state.totalCount.toString(), Colors.blue)),
               const SizedBox(width: 16),
               Expanded(child: _buildKpiCard(context, 'Ativos', state.activeCount.toString(), Colors.green)),
               const SizedBox(width: 16),
               Expanded(child: _buildKpiCard(context, 'Esgotados', state.outOfStockCount.toString(), Colors.red)),
               const SizedBox(width: 16),
               Expanded(child: _buildKpiCard(context, 'Em Promoção', state.onSaleCount.toString(), Colors.orange)),
             ],
           ),
           const SizedBox(height: 24),
  ...
  Widget _buildProductCard(BuildContext context, WidgetRef ref, Product product, List<Category> categories) {
    final currency = NumberFormat.simpleCurrency(locale: 'pt_BR');

    final imagePath = (product.images.isNotEmpty && product.mainImageIndex < product.images.length) 
              ...
              children: [
                Row(
                  children: [
                    ...
                    if (product.isOutOfStock) _statusBadge('Esgotado', Colors.red),
                ...
                Text('REF: ${product.reference}', style: TextStyle(color: Colors.grey.shade600, fontSize: 12)),
                const SizedBox(height: 4),
                Row(
                   mainAxisAlignment: MainAxisAlignment.spaceBetween,
                   children: [
                     Text(currency.format(product.retailPrice), style: const TextStyle(fontWeight: FontWeight.bold, color: Colors.blue)),
                     Text(currency.format(product.wholesalePrice), style: TextStyle(color: Colors.grey.shade600, fontSize: 12)),
                   ],
                ),
                const SizedBox(height: 8),
                Row(
