...
class SystemBackupService {
  ...
  Future<void> restoreFullBackup(File zipFile, {Function(double, String)? onProgress}) async {
    try {
      onProgress?.call(0.1, 'Extraindo arquivos do backup...');
      final (payload, extractDir) = await _packageService.preparePackage(zipFile);
      ...
      final syncedProducts = products.map((p) => p.copyWith(syncStatus: SyncStatus.synced)).toList();
      ...
      await _settingsRepo.saveSettings(settings.copyWith(isInitialSyncCompleted: true));
